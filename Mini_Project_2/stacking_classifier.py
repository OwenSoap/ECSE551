# -*- coding: utf-8 -*-
"""Stacking Classifier.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1OKCClcD9Qf2Z59egC0iTBDwc2xkamhsM
...
def tfidf_vectorizer(X_train, X_test, binary=False):
    stop_words = text.ENGLISH_STOP_WORDS
    tf_idf_vectorizer = TfidfVectorizer(binary=binary, stop_words=stop_words)
...
for train_index, test_index in kf.split(X):
    vectors_train, vectors_test = tfidf_vectorizer(X[train_index], X[test_index], binary=True)
    estimators = [
        ('mlp', MLPClassifier(max_iter=1000, learning_rate="adaptive", learning_rate_init=0.0001)),
        ('svc', svm.SVC(kernel='linear', gamma='auto', C=1, probability=True)),
        ('lr', LogisticRegression(C=40.0, max_iter=1000))
    ]
    ...
    accuracies.append(metrics.accuracy_score(y[test_index], y_test))
    print(accuracies[-1])
print("Average accuracy of Stacking Classification = {}".format(np.mean(accuracies)));
toc = time()
print("Time spent for Stacking Classification (with 5 fold validation) = {}".format(toc - tic))
...
for train_index, test_index in kf.split(X):
    vectors_train, vectors_test = tfidf_vectorizer(X[train_index], X[test_index], binary=True)
    clf1 = MLPClassifier(max_iter=1000, learning_rate="adaptive", learning_rate_init=0.0001)
    clf2 = LogisticRegression(C=40.0, max_iter=1000)
    clf3 = svm.SVC(kernel='linear', gamma='auto', C=1, probability=True)
    eclf = VotingClassifier(estimators=[('mlp', clf1), ('lr', clf2), ('svc', clf3)],
                            voting='soft', weights=[1,1,1])
    eclf = eclf.fit(vectors_train, y[train_index])
    y_test = eclf.predict(vectors_test)
    accuracies.append(metrics.accuracy_score(y[test_index], y_test))
    print(accuracies[-1])
print("Average accuracy of Volting Classification = {}".format(np.mean(accuracies)));
toc = time()
print("Time spent for Volting Classification (with 5 fold validation) = {}".format(toc - tic))